...
public class HorizontallyTimeLineNotFirstConstraint extends AdjustmentsPropagator {
    ...
    public HorizontallyTimeLineNotFirstConstraint(IntVar[] startingTimes_makespan, IntVar[] endingTimes, Integer[] heights, Integer[] processingTimes, int capacity) {
    ...
    public void propagate(int evtmask) throws ContradictionException {
        ...
        do
        {
            ...
            QuadHorizontallyNotFirst hnf = new QuadHorizontallyNotFirst(tasks, capacity);
            QuadHorizontallyNotFirst hnf_negative = new QuadHorizontallyNotFirst(negativeTasks, capacity);

            //TimeLineNotFirst qnf = new TimeLineNotFirst(tasks, capacity);
            //TimeLineNotFirst qnf_negative = new TimeLineNotFirst(negativeTasks, capacity);

            //The makespan is filtered
            if(hnf.makespan > makespan.getLB())
                makespan.updateLowerBound(hnf.makespan, this);
            //int[] estprimeQ = qnf.filterWithTimeline();
            //int[] estprime_negativeQ = qnf_negative.filterWithTimeline();
            ...
            for (int i = 0; i < nbTasks; i++)
            {
                ...
                /*if(estprimeQ[i] > startingTimes[i].getLB())
                {
                    //We update both set of tasks so that we don't have to reinitialize them.
                    startingTimes[i].updateLowerBound(estprimeQ[i], this);
                    negativeTasks[i].setLatestCompletionTime(-startingTimes[i].getLB() + maxLct);
                    fixpoint = false;
                    adjustments[i]++;
                }
                if(maxLct - estprime_negativeQ[i] - processingTimes[i] < startingTimes[i].getUB())
                {
                    //We update both set of tasks so that we don't have to reinitialize them.
                    startingTimes[i].updateUpperBound(maxLct - estprime_negativeQ[i] - processingTimes[i], this);
                    tasks[i].setLatestCompletionTime(startingTimes[i].getUB() + processingTimes[i]);
                    fixpoint = false;
                    adjustments[i]++;
                }*/
